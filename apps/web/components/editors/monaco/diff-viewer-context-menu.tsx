...
const itemCls =
  "flex w-full cursor-default select-none items-center gap-2 rounded-md px-2 py-1 text-xs outline-hidden focus:bg-muted focus:text-foreground hover:bg-muted hover:text-foreground";
    ...
    >
      ...
      {onRevert && (
        <>
          ...
          <button
            role="menuitem"
            className={`${itemCls} text-destructive focus:bg-destructive/10 focus:text-destructive hover:bg-destructive/10 hover:text-destructive`}
            onClick={() => {
              onRevert(filePath);
              onClose();
            }}
