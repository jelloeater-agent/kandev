...
Right-click any file or folder in the Files tree for **Open in \<editor\>**, which launches your default editor at that path instead of at the worktree root. When more than one editor is configured, **Open in other editor** lists the rest. In a multi-worktree task the tree is rooted above the worktrees, so Kandev resolves the clicked path back to its own worktree — no picker is needed. The action is hidden while several files are selected, because it applies to a single path.
